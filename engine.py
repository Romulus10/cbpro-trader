    ...
    def determine_trades(self, product_id, period_list, indicators):
        ...
        if self.is_live:
            ...
            for cur_period in period_list:
                new_buy_flag = new_buy_flag and Decimal(indicators[cur_period.name]['ema3']) > Decimal(indicators[cur_period.name]['ema6']) and \
                                                Decimal(indicators[cur_period.name]['ema3_trend']) > Decimal('0.0') and \
                                                Decimal(indicators[cur_period.name]['adx']) > Decimal('20.0')
                new_sell_flag = new_sell_flag or Decimal(indicators[cur_period.name]['ema3']) < Decimal(indicators[cur_period.name]['ema6']) or \
                                                 Decimal(indicators[cur_period.name]['adx']) < Decimal('20.0')
            ...
            if new_buy_flag:
                ...
                if not product.meta:
                    ...
                    if amount >= Decimal(product.min_size) and (time.time() - product.last_signal_switch) > 0.0:
                ...
                if not product.meta:
                    # Throttle to prevent flip flopping over trade signal
                    if amount_of_coin >= Decimal(product.min_size) and (time.time() - product.last_signal_switch) > 0.0:
                        if not product.order_in_progress:
